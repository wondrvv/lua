	-- =================================================================
	-- Nama Script : KAMUS SAKU V12 (FIXED SCHEDULER ERROR)
	-- Author      : wondrv x Duckiezt (Enhanced)
	-- Description : Robust Connection, Anti-Crash, Auto-Complete
	-- =================================================================
	...
	local CONFIG = {
		...
		-- Backup data jika HttpGet gagal (Scheduler Error)
	...
	SBox.PlaceholderText = "Sedang sinkronisasi..."
	SBox.Text = ""
	...
	ResText.Text = "Menghubungkan ke GitHub..."
	...
	-- SAFE LOADER (Mencegah Scheduler Error)
	task.spawn(function()
		local total = 0
		local seen = {}
		
		for _, url in ipairs(CONFIG.Sources) do
			-- Gunakan pcall agar jika HttpGet error, script tidak mati
		...
		-- Jika gagal ambil internet (total masih 0), pakai fallback
		if total == 0 then
			...
			Title.Text = "MODE: OFFLINE (Error)"
		else
			Title.Text = "KBBI V12 READY"
		end
